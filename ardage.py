...
def download_paper(paper_id, paper, output_dir):
    ...
    try:
        ...
        paper.download_pdf(dirpath=output_dir, filename=f"paper{paper_id}.pdf")
...
def _convert_worker(paper_id, pdf_dir, md_dir, result_queue):
    ...
    try:
        ...
        print("Error converting paper: ", e)
        result_queue.put(('error', paper_id, str(e)))


def convert_with_timeout(paper_id, pdf_dir, md_dir, timeout=120):
...
def main():
    ...
    number_of_papers = 10                   # Max papers (1-1000)
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        download_futures = {executor.submit(download_paper, i + 1, papers[i], output_dir_pdf): i + 1 
        ...
        for future in tqdm(as_completed(download_futures), total=len(download_futures), desc="Downloading"):
            try:
                ...
                print("Error in downloading ThreadPoolExecutor: ", e)
                pass
    ...
    def convert_wrapper(paper_id):
        result = convert_with_timeout(paper_id, output_dir_pdf, output_dir_md, timeout=120)
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        ...
        for future in tqdm(as_completed(conversion_futures), total=len(conversion_futures), desc="Converting"):
            try:
                ...
                print("Error in conversion ThreadPoolExecutor: ", e)
                pass
